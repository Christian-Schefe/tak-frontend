...
export class TakGameUI {
  ...
  updateGame(game: TakBaseGame) {
    this.actualGame = game;
    this.partialAction = null;
    this.priorityPieces = getLastActionPiecesInOrder(game);
    this.onGameUpdate();
  }

  setPlyIndex(index: number | null) {
    const prevIndex = this.plyIndex ?? this.actualGame.actionHistory.length;
    this.plyIndex =
      index === null || index >= this.actualGame.actionHistory.length ? null : Math.max(0, index);

    const newIndex = this.plyIndex ?? this.actualGame.actionHistory.length;
    ...
    this.priorityPieces = isSteppingForwardOne
      ? this.actualGame.actionHistory[prevIndex].pieceIds
      : isSteppingBackOne
        ? this.actualGame.actionHistory[newIndex].pieceIds
        : [];
    this.partialAction = null;
    this.onGameUpdate();
  }

  private getShownGame(): TakBaseGame {
    const gameClone = isDraft(this.actualGame)
      ? current(this.actualGame).clone()
      : this.actualGame.clone();
    const shownGame = this.plyIndex !== null ? gameClone.trimToPlyCount(this.plyIndex) : gameClone;
    return shownGame;
  }

  private onGameUpdate() {
    const shownGame = this.getShownGame();
    ...
    const isNotHistoric = this.plyIndex === null;
    ...
    if ((this.plyIndex ?? this.actualGame.actionHistory.length) >= 1) {
      const lastAction =
        this.actualGame.actionHistory[
          this.plyIndex !== null ? this.plyIndex - 1 : this.actualGame.actionHistory.length - 1
        ];
